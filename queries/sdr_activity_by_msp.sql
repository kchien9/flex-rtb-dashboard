...
-- 49.6% of current accounts -- OR'ing in ACCOUNT_HUBSPOT_ID = ACCOUNT_ID raised that to 90.6%
-- (114,819 of 126,675 current accounts resolved to a real DIM_SALES_ACCOUNTS row). Checked for
-- fan-out from the OR: only 1 of 126,674 CRM_ACCOUNT_SK values matches more than one
-- DIM_SALES_ACCOUNTS row -- negligible, not a real fan-out risk.
--
...
--
-- VALIDATED LIVE 2026-08-06 (GraceMonths=3, LookbackMonths=6): 17 real MSP names appear
-- (AppFolio, Entrata, MRI, RealPage, Buildium, etc) plus 'Not Set' for unresolved accounts --
-- the join is not broken. NOTE: unresolved-MSP calls are NOT dropped by this file, they're
-- bucketed into 'Not Set' and kept, so the only source of drop-off vs. sdr_funnel_by_segment.sql
-- is the DSMB/750-unit exclusion that file's own activity CTE doesn't apply (only its pipeline
-- CTE does). Actual measured drop-off is small: July 2026 (most recent full month) calls =
-- 8,569 here vs. 8,794 in sdr_funnel_by_segment.sql, i.e. 97.4% retained (2.6% lost to DSMB
-- exclusion). Feb/Mar 2026 matched exactly (0% loss) -- the gap grows to ~3.2% by August as
-- small-PMC call volume rises -- all well under the 50%-loss concern threshold, confirms no
-- join problem.
