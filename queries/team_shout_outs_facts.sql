...
--
-- ONE COMBINED QUERY, NOT THREE SEPARATE STATEMENTS -- restructured 2026-08-05: the Shout Outs
-- prompt tooling wires exactly ONE data source per name (same as shout_outs_facts.sql, which is
-- one query), so the original 3-statement version (Part A/B/C as separate SELECTs) has been
-- merged into a single UNION ALL below. All three grains share the same output shape --
-- fact_type, entity, this_period_value, last_period_value, mom_ratio, mom_pct_change, is_high,
-- prior_max_value -- so the LLM prompt can treat every row identically regardless of grain.
--
...
--   - Whole-org units (org_units): +20.8% MoM (337,707 vs 279,690) -- real, lines up with
--     Kevin's own "20% more units" example almost exactly.
--   - AE team units (team_units): all 4 teams up 15-31% MoM. AND a real is_high hit:
--     Sebastian's Team's current month (47,933 units) is genuinely its highest EVER on record
--     (prior max 45,914) -- a legitimate "all-time high" story, not a fixed-window artifact,
--     because this check uses full history same as the fix just applied to shout_outs_facts.sql.
--   - SDR pod activity (sdr_calls / sdr_meetings_booked / sdr_meetings_held): genuinely DOWN,
--     not up, this period. Calls flat-to-down (-4% to +1%), meetings_booked down 50-80% across
--     all 3 pods (comparing the last two FULLY COMPLETE calendar months -- see that section's
--     comment for why the current partial month is excluded entirely, not just flagged). No pod
--     is currently "hitting a high" on calls or meetings_booked against its own history either.
--     This file surfaces that honestly -- it must never force an SDR win that isn't real.
--
...
WITH
-- ================= AE TEAM UNITS (fact_type = 'team_units') =================
-- Same BP-month anchor as shout_outs_facts.sql (MAX(BP_MONTH) = "this month," always a settled,
-- non-partial month). Same DSMB exclusion, same team_bucket mapping via HUBSPOT_DEAL_OWNER ->
-- STG_SALESFORCE__USER.TEAM_NAME used throughout this repo.
a_pmc_size AS (
...
a_team_history AS (
    ...
    LEFT JOIN a_pmc_size p ON s.PMC_ID = p.PMC_ID
    JOIN a_current_rep cr ON cr.FULL_NAME = s.HUBSPOT_DEAL_OWNER AND cr.team_bucket IS NOT NULL
...
-- ================= SDR POD ACTIVITY (calls, meetings booked, meetings held) =================
-- DELIBERATE DEPARTURE FROM THE USUAL "FLAG, DON'T HIDE" PARTIAL-MONTH RULE. Every other file
-- in this repo (sdr_funnel_by_segment.sql, sdr_activity_by_rep.sql) flags the current
-- in-progress calendar month with `is_partial_month` and still shows it, because a dashboard
-- viewer can read the flag in context. A Slack shoutout has no such context -- it's a
-- broadcast, read once, with no filter UI. Comparing a few-days-old current month against a
-- full prior month would silently read as "activity collapsed," which is false (confirmed live:
-- doing this naively showed calls "down" 90%+ purely because the current month had 5 days of
-- data). So this block SKIPS the current partial calendar month entirely and compares the last
-- TWO FULLY COMPLETE calendar months instead, and uses FULL available history (every complete
-- month on record) for the is_high check, same full-history principle as the team-units block.
--
...
--
-- CALLS/BOOKED/HELD ALL FLOORED SEPARATELY ON LAST-PERIOD VOLUME -- meetings_booked/held are
-- routinely single-to-low-double-digit, too small a base to put a clean percent on, while calls
-- is a much steadier base. Floors: calls last-period >= 200, meetings_booked/held last-period
-- >= 10. Expect the meetings facts to be genuinely absent some periods -- that's the floor
...
b_emp AS (
    SELECT ed.EMPLOYEE_SK,
        ...
        END AS sdr_segment
    FROM b_emp_dedup ed
    JOIN b_user_dedup u ON ed.EMAIL = u.EMAIL
...
b_activity_history AS (
    ...
    JOIN b_emp e ON t.EMPLOYEE_SK = e.EMPLOYEE_SK AND e.sdr_segment IS NOT NULL
    WHERE t.TASK_STATUS = 'completed'
      AND t.COMPLETED_AT_UTC < DATE_TRUNC('month', CURRENT_DATE())
    GROUP BY 1, 2
),
b_meetings_history AS (
    ...
    JOIN b_emp e ON m.EMPLOYEE_SK = e.EMPLOYEE_SK AND e.sdr_segment IS NOT NULL
...
b_combined AS (
    ...
    FROM b_activity_history a
    FULL OUTER JOIN b_meetings_history mt ON a.mo = mt.mo AND a.sdr_segment = mt.sdr_segment
...
b_per_pod AS (
    ...
    FROM b_combined c
    CROSS JOIN b_target_months tm
    GROUP BY c.sdr_segment
),
-- ================= WHOLE ORG UNITS (fact_type = 'org_units') =================
-- Same anchor/exclusion pattern as the team-units block, just without the team join -- every
-- DSMB-excluded rolled-out unit company-wide, not restricted to reps currently matched to one
-- of the 4 named AE teams (a team-restricted sum would silently understate the real company
-- total by however many units belong to unmatched/departed reps).
c_pmc_size AS (
    SELECT PMC_ID, SUM(PROPERTY_UNIT_COUNT) AS pmc_current_units
    FROM PRODUCTION.ANALYTICS.PROPERTY_BP_MONTH_STATS
    WHERE BP_MONTH = (SELECT MAX(BP_MONTH) FROM PRODUCTION.ANALYTICS.PROPERTY_BP_MONTH_STATS)
...
c_org_history AS (
    SELECT s.BP_MONTH,
        SUM(IFF(s.IS_NEW_INTEGRATED OR s.IS_RECAPTURED_NEW_ROLLOUT OR s.IS_RECAPTURED_OTHER, s.PROPERTY_UNIT_COUNT, 0)) AS units
    FROM PRODUCTION.ANALYTICS.PROPERTY_BP_MONTH_STATS s
    LEFT JOIN c_pmc_size p ON s.PMC_ID = p.PMC_ID
    WHERE (p.pmc_current_units IS NULL OR p.pmc_current_units > 750)
    GROUP BY 1
),
c_org AS (
...
-- ================= FINAL: UNION ALL, ONE CONSISTENT SHAPE =================
SELECT
...
FROM b_per_pod WHERE COALESCE(calls_last, 0) >= 200
...
-- calls_prior_max, not an accidental copy), only the alias is shared on purpose so every
-- fact_type exposes the same column name for "this metric's best-ever period."
SELECT 'sdr_meetings_booked' AS fact_type, sdr_segment AS entity,
    ...
    booked_prior_max
FROM b_per_pod WHERE COALESCE(booked_last, 0) >= 10
...
SELECT 'sdr_meetings_held' AS fact_type, sdr_segment AS entity,
    ...
    held_prior_max
FROM b_per_pod WHERE COALESCE(held_last, 0) >= 10
...
SELECT
    ...
    this_period_value,
    last_period_value,
    IFF(COALESCE(last_period_value, 0) > 0, this_period_value / last_period_value, NULL),
    IFF(COALESCE(last_period_value, 0) > 0, (this_period_value - last_period_value) / last_period_value, NULL),
    IFF(COALESCE(this_period_value, 0) > 0 AND prior_period_count >= 2 AND this_period_value > COALESCE(prior_max_value, -1), TRUE, FALSE),
    prior_max_value
FROM c_org

ORDER BY fact_type, entity;
